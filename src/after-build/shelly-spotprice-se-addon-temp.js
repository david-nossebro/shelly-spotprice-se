...
function USER_OVERRIDE(inst, cmd, callback) {
  ...
  try {
    ...
    if (temp.tC === null) {
      state.si[inst].str =
        'Temperature control error - is the sensor connected?';
      throw new Error('Is the sensor connected?');
    }

    if (cmd && temp.tC > 15) {
      state.si[inst].str =
        'Temperature ' + temp.tC + '°C is over 15°C -> control off';
      console.log(
        'Temperature is over 15 degrees, setting control off. Temperature now:',
        temp.tC
      );
      cmd = false;
    } else if (!cmd && temp.tC < 5) {
      state.si[inst].str =
        'Temperature ' + temp.tC + '°C is below 5°C -> control on';
      console.log(
        'Temperature is below 5 degrees, setting control on. Temperature now:',
        temp.tC
      );
      cmd = true;
    } else {
      state.si[inst].str =
        'Temperature ' + temp.tC + '°C -> following the control logic';
    }
