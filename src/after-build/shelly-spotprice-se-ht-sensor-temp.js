...
const TEMPERATURE_MAX_AGE_HOURS = 12;
...
function USER_CONFIG(inst, initialized) {
  ...
  try {
    if (data === null) {
      ...
      state.si[inst].str =
        'Temperature unknown -> cheap hours: ' +
        hours +
        ' h, control: ' +
        minutes +
        ' min';
    } else {
      const age = (Date.now() - data.ts) / 1000.0 / 60.0 / 60.0;
      console.log(
        'Temperature is known (updated ' + age.toFixed(2) + ' h ago):',
        data
      );
...
function onHttpRequest(request, response) {
  try {
    ...
    if (params.temp !== undefined) {
      ...
      console.log(
        "Failed to update temperature data, 'temp' is missing from parameters:",
        params
      );
      response.code = 400;
    }
